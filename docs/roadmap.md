...
Last reviewed: 2026-09-22. Current position: **through 5.6 hierarchical memory**; next numbered task is **5.7**.
...
| 5 | Text assistant | **5.6 hierarchical memory implemented**; UI polish and agent loop remain |
...
Status: **in progress through 5.6**; **5.7 is next**.
...
- **5.5** Citations + safety — **implemented:** claim risk, withhold unverified critical specs,
  ...
  generator yet
- **5.6** Hierarchical memory — **implemented:** search bike-scoped rolling summaries, expand
  bounded raw spans on hit (`HierarchicalMemoryService` + tools); lexical ranking until summary
