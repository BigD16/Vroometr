...
Next.js App Router UI. The garage HUD, Garage pages, and dashboard load owner-scoped bike rows through FastAPI. Garage supports combustion and electric bikes across list, create, dedicated detail, edit, archive, and restore flows. Dashboard identity, powertrain, engine hours, and Assistant context are live; maintenance and ride areas show empty states until those domains are implemented. Assistant (5.1–5.7) lists bike-scoped threads, stores user messages, records bike-context switches, shows compact context, polished composer/messages/source-chip UI, shell FAB, and disclaimer—there are no agent replies yet. Documents supports direct private uploads, active-bike and account file lists, sandboxed previews/downloads, unlink/relink, and confirmed file deletion. Account usage includes pending uploads. New uploads become deletable when their 15-minute upload grant expires.
...
| Assistant conversations (5.1–5.7) | `components/AssistantWorkspace.tsx`, `components/AssistantSources.tsx`, `components/AssistantFab.tsx`, `app/api/conversations/` |
