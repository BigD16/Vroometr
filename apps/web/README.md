...
Next.js App Router UI. The garage HUD, Garage pages, and dashboard load owner-scoped bike rows through FastAPI. Garage supports combustion and electric bikes across list, create, dedicated detail, edit, archive, and restore flows. Dashboard identity, powertrain, engine hours, and Assistant context are live; maintenance and ride areas show empty states until those domains are implemented. Assistant (5.1–5.7 + ReasoningAgent) lists bike-scoped threads, stores messages, can reply when `AGENT_MODEL` is configured, shows compact context and Sources chips, shell FAB, and disclaimer. Documents supports direct private uploads, active-bike and account file lists, sandboxed previews/downloads, unlink/relink, and confirmed file deletion. Account usage includes pending uploads. New uploads become deletable when their 15-minute upload grant expires.
